...
struct single_number *create_single_number(double *number, unsigned short level) {
    struct single_number *num_ptr = malloc(sizeof(struct single_number));
    ...
    if (num_ptr->current_op >= SINGLE_END) {
        ...
        if (num_ptr->wrapped_number == NULL) {
            if (num_ptr->level <= 0) {
                ...
                struct single_number *new_num = create_single_number(num_ptr->num_ptr, (unsigned short) (num_ptr->level - 1));
